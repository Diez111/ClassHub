...
require_once ROOT . '/mvc/modelos/chat_publico/chat_publicomodel.php'; //carga el archivo con el modelo
require_once ROOT . '/mvc/system/session.php'; //carga el archivo de la clase 'session'

class chat_publicocontroller extends controller  //Crea la clase "chat_publicocontroller" con herencia de la clase "controller"
{
	...
	{
		$this->model = new chat_publicomodel(); //creamos el objeto 'model' con la clase 'chat_publicomodel' para poder comunicarse con la base de datos a traves del modelo
	...
	public function enviar_mensaje($parametros) //función para enviar un mensaje
	{
		if (!empty($parametros['mensaje'])) { //verifica que no haya datos vacíos
			...
			$str = 'location: /mvc/chat_publico/exec/' . $parametros['curso_id']; //creamos una ruta para la función 'header' y simular una recarga de página, el parametro que le pasamos es la id de la persona con la que hablamos
	...
	public function exec($curso) //función que se encarga de obtener los datos para la vista y mostrarla, el parametro que recibimos es de la URL
	{
		$query = $this->model->mensajes($curso); //le decimos al modelo que nos de el historial de mensajes de una conversación, según la id del curso que recibimos como parametro
		
		$params = array('nombre' => $this->session->get('nombre'), 'rol' => $this->session->get('rol'), 'tema' => $this->session->get('temavis'), 'id_curso' => $curso, 'id_usuario' => $this->session->get('idUser'), 'query' => $query); //asignamos los parametros que vamos a enviar a la vista
