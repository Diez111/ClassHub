...
require_once ROOT . '/mvc/modelos/examen_menu_admin/examen_menu_adminmodel.php'; //carga el archivo con el modelo
require_once ROOT . '/mvc/system/session.php'; //carga el archivo de la clase 'session'

class examen_menu_admincontroller extends controller  //Crea la clase "examen_menu_admincontroller" con herencia de la clase "controller"
{
	...
	{
		$this->model = new examen_menu_adminmodel(); //creamos el objeto 'model' con la clase 'examen_menu_adminmodel' para poder comunicarse con la base de datos a traves del modelo
		...
		if ($this->session->get('rol') == 3) { //si el rol es diferente a 3(alumno) se puede acceder a la página
			header('location: /mvc/errorpage'); //sino nos manda a la página de error
		}
	}
