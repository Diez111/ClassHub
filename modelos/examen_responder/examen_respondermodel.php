...
class examen_respondermodel extends model
{
	...
	public function corregirPrueba($parametros)
	{
		...
		$sql = "SELECT nota FROM exam_notas WHERE user_id = '$idusuario' AND test_id = '$idprueba'";
		$query = $this->db->query($sql);
		if (!($query->num_rows)) {

			$sql = "SELECT total_que FROM exam_test WHERE test_id = '$idprueba'";
			$query = $this->db->query($sql);
			$dato = $query->fetch_assoc();
			$cant_quest = $dato['total_que'];
