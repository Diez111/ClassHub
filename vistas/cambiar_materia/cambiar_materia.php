...
<head>
  ...
  <title>Cambiar materia</title><!--Titulo de la pestaña del navegador -->
</head>
<body>
  ...
  <br><br><br><br><br><br><br><!-- Saltos de linea -->
    <div>
      <center><h1>Actualizar materia</h1></center><!-- Titulo -->
      <form action="<?= FOLDER_PATH . '/cambiar_materia/cambiar_nombre/'?>" method="post"><!-- Le pasamos los datos al controller mediante el método POST para que el envío de datos no sea visible para el usuario-->
        <input type="hidden" name="id_materia" value="<?= $id_materia ?>">
        <label for="materia">Nuevo nombre del materia</label><!-- Texto -->
        <input type="text" name="materia" id="materia" placeholder="Ingrese un nombre" value="<?= $materia ?>" required><!-- En este campo se pone el nombre -->
